...
$result4 = mysql_query("SELECT  body, cat_id, up, down, id, user_id, time_stamp FROM content WHERE id=$post_id");
while ( $row = mysql_fetch_row($result4) ) {
    ...
    if ($i_tag==1){
    	echo('<p><img src="images/suncreen.jpeg" alt="Profile Photo" with=200px></p>');
    }
    echo('   <p>'.$body.'</p>
...
$tweeturl = $json->id;
    ...
    echo("<iframe src='//www.facebook.com/plugins/like.php?href=$longUrl&amp;send=$body&amp;layout=button_count&amp;width=450&amp;show_faces=false&amp;action=like&amp;colorscheme=dark&amp;font=arial&amp;height=21' scrolling='no' frameborder='0' style='border:none; overflow:hidden; width:450px; height:21px;' allowTransparency='true'></iframe>");
    echo(' </li>');
